...
boot_alloc(uint32_t n)
{
	...
	char *result;
	physaddr_t endaddress = npages*PGSIZE;

	// Initialize nextfree if this is the first time.
	// 'end' is a magic [-----P-] CPL=0 II=0 symbol automatically generated by the linker,
	...
	//cprintf("n is %u, nextfree is %u\n", n, PADDR(nextfree));
	result = nextfree;
	if(n == 0) return result;
	nextfree += PGSIZE;	
	while(n > PGSIZE && (PADDR(nextfree) < endaddress)){
		nextfree += PGSIZE;
		n -= PGSIZE;	
	}
        if(PADDR(nextfree) < endaddress)
            return result;
        panic("boot_alloc: This function is out of memory\n");
        return NULL;
}
...
page_init(void)
{
	...
	for (i = 1; i < npages; i++) {
		if ( (i < npages_basemem) || ( i >= (PADDR(first_free_page)/PGSIZE)) ){
		    pages[i].pp_ref = 0;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(pte) return (pte_t*)KADDR((physaddr_t)(pte + PTX(va)));
	if(!create) return NULL;
	struct Page* pp = page_alloc(ALLOC_ZERO);
	if(!pp) return NULL;
	pp->pp_ref += 1;
	pgdir[PDX(va)] = page2pa(pp)|PTE_P|PTE_W;
	return (pte_t*)KADDR((physaddr_t)((pte_t*)page2pa(pp)+PTX(va)));
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for( ; size; size-=PGSIZE){
		pte = (pte_t*) pgdir_walk(pgdir, (void *)(va+size-PGSIZE), 1);
		*pte = (pa+size-PGSIZE)|perm|PTE_P;
	}
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pte_t * pte = (pte_t*)PTE_ADDR(pgdir_walk(pgdir, va, 1));
	if(!pte) return -E_NO_MEM;
	pgdir[PDX(va)] = (pde_t)PADDR(pte)|perm|PTE_P;
	pte[PTX(va)] = page2pa(pp)|perm|PTE_P;
	pp->pp_ref ++;
	if(pp == page_free_list) page_free_list = page_free_list->pp_link;
...
{
	// Fill this function in
	pte_t * pte = (pte_t*)PTE_ADDR(pgdir_walk(pgdir, va, 0));
	if(pte){
		physaddr_t page = pte[PTX(va)];	
		if(page){
			if(pte_store){
				*pte_store = pte;	
			} 
